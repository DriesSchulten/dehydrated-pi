...
def _create_path(file, ext):
    paths = file.split("/")[:-1]
    filename = "/" + paths.pop(-1).replace("star_", "").replace("_", ".") + "." + ext
...
def deploy_cert(args):
    ...
    logger.debug(' + ssl_certificate_chain: {0}'.format(chain_pem))

    shutil.copy(privkey_pem, _create_path(privkey_pem, "key"))
    shutil.copy(fullchain_pem, _create_path(fullchain_pem, "crt"))
